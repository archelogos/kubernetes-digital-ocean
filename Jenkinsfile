...
node {
  ...
  try {
    timeout(time: 15, unit: 'SECONDS') {
      forceCompleteDeploy = input(
        id: 'Proceed0', message: 'Force Deployment', parameters: [
  ...
  try {
    ...
    stage('Push Docker') {
      withCredentials([usernamePassword(credentialsId: 'User Docker Hub', usernameVariable: 'DOCKER_HUB_USERNAME', passwordVariable: 'DOCKER_HUB_PASSWORD')]) {
    ...
    // Notify Success
    slackSend (color: '#00FF00', channel: '#api', message: "SUCCESSFUL: Job '${env.JOB_NAME} [${env.BUILD_NUMBER}]' (${env.BUILD_URL})")
    emailext (
    ...
    // Notify Error
    slackSend (color: '#FF0000', channel: '#api', message: "FAILED: Job '${env.JOB_NAME} [${env.BUILD_NUMBER}]' (${env.BUILD_URL})")
    emailext (
